...
const worker = require('./go_worker');
...
exports.clone = async function (repo, sdk, url) {
  if (await exists(sdk)) {
   ...
   await console.log('sdk already exits......');
    return;
  }
  //clone(repo, path.join(repo, 'go/src/github.com/aliyun/alibaba-cloud-sdk-go'), `https://github.com/${GITHUB_USERNAME}/alibaba-cloud-sdk-go.git`),
...
async function release(task, gitPath, release = false, options) {
  const { ctx } = this;
  await console.log('start release.......');
...
async function getReleaseTag(task) {
  ...
  const releaseFile = path.join(config['output'], task.taskId, 'alibaba-cloud-sdk-go', 'ChangeLog.txt');
  await console.log(releaseFile);
  const releaseContent = await readFile(releaseFile, 'utf8');
  const version = releaseContent.match(/[0-9]{4}-[0-9]{2}-[0-9]{2} Version: (\S*)/)[1];
  const newVersionArray = version.split('.');
  ++newVersionArray[2];
  const newVersion = newVersionArray.join('.');
  task.version = newVersion;
  await console.log('end getreleaseTag');
  return task.version;
}

async function updateChangeLog(task, gitPath) {
  await console.log('start updatechangelog....');
  const changelog = path.join(config['output'], task.taskId, 'alibaba-cloud-sdk-go', 'ChangeLog.txt');
  const defaultLog = '- Generated ' + task.apiVersions.join(', ') + ' for `' + task.productId + '`.';
