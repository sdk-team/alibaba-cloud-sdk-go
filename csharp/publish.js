...
const worker = require('./csharp_worker');
...
exports.preBuild = async function (task, sdkCoreDir, options) {
  await console.log('start prebuilding......');
  ...
  await console.log('end prebuilding.........');
}
...
exports.publishSdkToGithub = async function (task, gitPath, options, isRelease = false) {
  try {
    ...
    await release(task, task.gitCodePath, isRelease, options);
    const result = await exec('git rev-parse HEAD',
      Object.assign({ cwd: task.gitCodePath }, options));//commit id
...
async function release(task, gitPath, release = false, options) {
  ...
  options = Object.assign({ cwd: task.gitCodePath }, options);
